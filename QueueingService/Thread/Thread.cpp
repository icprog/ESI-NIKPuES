...
DWORD WINAPI ServerServerThread(LPVOID lpParam)
{
	...
	*/
	...
	hints.ai_flags = AI_PASSIVE;     // 
									 ...
									 // Resolve the server address and port
	iResult = getaddrinfo(NULL, SERVER_PORT, &hints, &resultingAddress);           // Poseban port za komunikaciju dva servera
	if (iResult != 0)
	{
		printf("getaddrinfo failed with error: %d\n", iResult);
		WSACleanup();
	...
	do
	{
		...
		/*
			...
			Kada servis prima poruku od drugog servisa, on aktivira novu nit koja puni odgovarajuci bafer.

		*/
		PushParams params;
		params.queue = queue;
		MySocket serverServerSocket;
		serverServerSocket.bufferName = NULL;
		serverServerSocket.socket = acceptedSocket;
		params.socket = &serverServerSocket;
		params.initializator = 0;
		// pronadji prvi slobodan tred i kreiraj novi
		for (int i = 3; i < threadArray->size; i++) {
			if (threadArray->threads[i] == NULL) {
				DWORD threadID;
